let
    // ============================================================
    // EFD_Extraida (processamento completo do EFD, incluindo hierarquia e chaves relacionais)
    Cfg = Config,
    PastasRaw =
        let
    ...
    CombineColumnsPaths = Table.CombineColumns(
    ...
    AddDataInicialArquivo = Table.AddColumn(
        ...
        (r as record) as nullable date =>
            let
                t = r[ColunaTabelasSped], d = try List.First(List.RemoveNulls(t[DataInicial])) otherwise null
            in
                d,
        type date
    ),
    // 2) Ordena os arquivos por essa data
    Ordenado = Table.Sort(AddDataInicialArquivo, {{"dataInicialArquivo", Order.Ascending}}),
    ...
    AdicionaChaveLinha = Table.AddColumn(
        AlteraTipo, "Chave_Linha", each Text.From([ArquivoId]) & "-" & Text.From([LinhaId]), type text
    ),
    // -> Chave Estrangeira (Foreign Key): ID do Arquivo + ID da Linha do Pai
    AdicionaChavePai = Table.AddColumn(
        AdicionaChaveLinha,
        "Chave_Pai",
        each if [Linha_Pai] <> null then Text.From([ArquivoId]) & "-" & Text.From([Linha_Pai]) else null,
        type text
    ),
    // ... restante do código acima ...
    // 5. LIMPEZA FINAL
    RemoveTemporarias = Table.RemoveColumns(AdicionaChavePai, {"Linha_Pai"}),
    Reordena01 = Table.ReorderColumns(
        RemoveTemporarias,
        {
            "Nome",
            "Periodo",
